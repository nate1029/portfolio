...
/* ---------- scroll-linked grid warp (mobile) ---------- */
...
if (!canHover && !reduced) {
  ...
  if (canvas) {
    ...
    const GAP = 36, SEG = 9, RADIUS = 200, PUSH = 30;
    let W, H, dpr;
    let lastScroll = 0, scrollV = 0;
    // warp origin tracks horizontally across screen based on scroll velocity
    const warpPt_pos = { x: -9999, y: -9999 };
    const soft = { x: -9999, y: -9999 };
    ...
    addEventListener("scroll", () => {
      ...
      scrollV = y - lastScroll;
      lastScroll = y;
      // warp origin = center-x, vertical position moves with scroll in viewport
      const clampedV = Math.max(-60, Math.min(60, scrollV));
      warpPt_pos.x = W / 2 + (clampedV / 60) * (W * 0.35);
      warpPt_pos.y = H / 2;
    }, { passive: true });

    function calcWarp(x, y) {
    ...
    function frame() {
      // drift back to center when not scrolling
      soft.x += (warpPt_pos.x - soft.x) * 0.08;
      soft.y += (warpPt_pos.y - soft.y) * 0.08;
      warpPt_pos.x += (W / 2 - warpPt_pos.x) * 0.05;
      ...
      for (let x = 0; x <= W + GAP; x += GAP) {
        const pts = [];
        for (let y = 0; y <= H + SEG; y += SEG) pts.push(calcWarp(x, y));
        smoothLine(pts);
      }
      for (let y = 0; y <= H + GAP; y += GAP) {
        const pts = [];
        for (let x = 0; x <= W + SEG; x += SEG) pts.push(calcWarp(x, y));
        smoothLine(pts);
      }
      requestAnimationFrame(frame);
    }
